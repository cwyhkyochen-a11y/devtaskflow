...
- **零配置启动** — 自动从 OpenClaw 环境读取 LLM 配置，无需手动填写 API Key
- **架构重构** — pipeline core 与编排器解耦，local_llm / openclaw_subagent 双模式无缝切换
- **Git 自动化** — 新建项目自动 `git init`，write/fix 阶段自动 commit，seal 前自动确保 git 仓库

### v0.9.0 新增

- **错误恢复机制** — 三层保障：指数退避自动重试 + 检查点快照回滚 + 自动检测修复状态异常（`dtflow advanced recover` / `rollback`）
- **配置三档模式** — 极简（自动检测已有配置）/ 引导（3 步完成）/ 高级（完整配置），`dtflow setup --mode auto|guided|advanced`
- **封版自动化** — 封版自动生成 CHANGELOG、自动创建下一版本目录、DEPLOYMENT.md 从配置自动填充
- **友好错误提示** — 14 种错误类型覆盖，每种给出具体操作建议
